...
namespace TestPlatform.Application.Questions.Tags.Features.CreateTagCommand;
...
public class CreateTagHandler : ICommandHandler<CreateTagCommand, Guid>
{
    ...
    public async Task<Result<Guid>> Handle(CreateTagCommand command, CancellationToken cancellationToken)
    {
        ...
        if (await _tagsRepository.ExistsByNameAsync(null, tagResult.Value.Name, cancellationToken))
            return Result.Failure<Guid>(ErrorCodes.TagAlreadyExists);
