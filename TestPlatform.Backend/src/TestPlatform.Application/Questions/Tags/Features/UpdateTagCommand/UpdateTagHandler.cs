...
namespace TestPlatform.Application.Questions.Tags.Features.UpdateTagCommand;
...
public class UpdateTagHandler : ICommandHandler<UpdateTagCommand>
{
    ...
    public async Task<Result> Handle(UpdateTagCommand command, CancellationToken cancellationToken)
    {
        ...
        if (await _tagsRepository.ExistsByNameAsync(tag.Id, tag.Name, cancellationToken))
            return Result.Failure(ErrorCodes.TagAlreadyExists);
