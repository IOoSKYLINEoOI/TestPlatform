...
namespace TestPlatform.Core.Tags;
...
public class Tag
{
    ...
    public static Result<Tag> Create(string name, string description)
    {
        ...
        return Result.Success(new Tag(Guid.NewGuid(), name, description));
    }

    public static Result<Tag> CreateWithId(Guid id, string name, string description)
    {
        var validation = Validate(name, description);
        if (validation.IsFailure)
            return Result.Failure<Tag>(validation.Error);

        return Result.Success(new Tag(id, name, description));
    }
